...
# Generic type parameters for the input data-sample type and the result type
# so that subclasses can specialise `analyze_batch` without breaking the base
# contract (each analysis family works on its own sample/result pair).
S = TypeVar("S")
R = TypeVar("R")


class ErrorAnalysis(ABC, Generic[S, R]):
    ...
    It is generic over the input data-sample type ``S`` and the result type
    ``R`` so concrete implementations can commit to their own types.
    ...
    @abstractmethod
    def analyze_batch(self, data_samples: list[S]) -> R:
        """Analyze a batch of error-analysis data samples and return clustered results.

        Concrete subclasses implement this with their specific data-sample and
        result types (subgoal- or tool-call-oriented). It is the primary
        synchronous entry point for every error-analysis implementation.

        :param data_samples: the error-analysis data samples to analyze.
        :return: an error-analysis result with errors grouped by cluster.
        """
        ...
