...
from agent_inspect.exception import InvalidInputValueError, ToolError
...
class SubgoalErrorAnalysis(
    ErrorAnalysis[SubgoalErrorAnalysisDataSample, SubgoalErrorAnalysisResult]
):
    ...
    async def _analyze(
        ...
        base_errors: list[str | None] = await asyncio.gather(*tasks)
        ...
        analyzed_subgoal_validations = [
            ...
            for subgoal_validation, base_error in zip(
                data_sample.subgoal_validations, base_errors, strict=True
            )
        ]
    ...
    def _build_clustered_result(
        ...
        assigned_error_ids: set[int] = set()
