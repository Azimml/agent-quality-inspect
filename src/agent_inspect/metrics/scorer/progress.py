...
from agent_inspect.metrics.constants import INCLUDE_VALIDATION_RESULTS, MAX_TURNS, MAX_TURNS_DEFAULT
    ...
    def evaluate(
        ...
        for idx, turns_grouping in enumerate(turns_groupings):
            ...
            for current_subgoal in current_subgoals:
                ...
                # NOTE: subgoal.details is used as the explanation key; if two subgoals
                # share identical details the later one overwrites the earlier entry.
...
class ProgressScoresThroughTurns(ProgressBasedMetric):
    ...
    def evaluate(
        ...
        for turns_grouping in turns_groupings:
        ...
        score_objs = []
        for score, judge_exp in zip(scores, judge_explanations_turns, strict=True):
